...
/**
 * 녹화 상태 배지 표시 (화면 우상단 빨간색 REC 표시)
 */
function _showRecBadge() {
  if (_recBadge) return;
  _recBadge = document.createElement('div');
  _recBadge.id = 'rec-badge';
  _recBadge.style.cssText = 'position:fixed;top:60px;right:16px;background:#dc2626;color:#fff;padding:8px 14px;border-radius:20px;font-weight:600;font-size:13px;z-index:9999;box-shadow:0 4px 12px rgba(220,38,38,0.4);display:flex;align-items:center;gap:6px;pointer-events:none;';
  _recBadge.innerHTML = '<span style="width:8px;height:8px;background:#fff;border-radius:50%;animation:rec-blink 1s infinite;"></span><span id="rec-time">REC 00:00</span>';
  document.body.appendChild(_recBadge);

  if (!document.getElementById('rec-blink-style')) {
    var s = document.createElement('style');
    s.id = 'rec-blink-style';
    s.textContent = '@keyframes rec-blink{0%,100%{opacity:1}50%{opacity:0.3}}';
