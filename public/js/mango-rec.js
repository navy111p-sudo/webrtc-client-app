...
(function () {
  ...
  function hookVcLeave() {
    ...
    window.vcLeaveRoom = async function () {
      // 녹화 중이면 먼저 종료하고 R2 업로드 완료 대기
      if (isRecording) {
        console.log('[mango-rec] 수업 종료 → 녹화 자동 중지');
        stopRecording();
        // mediaRecorder.onstop은 비동기로 R2 업로드를 완료함
        // 짧은 대기 후 원래 leaveRoom 실행
        await new Promise(r => setTimeout(r, 500));
      }
      origLeave.apply(this, arguments);
    };
